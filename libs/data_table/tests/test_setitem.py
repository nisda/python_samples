...
def test_getitem_no_column():
    ...
    assert dt.columns == None   # 元が None なので追加されない
...
def test_getitem_no_column_zero():
    ...
    # 列追加の処理をするためデータ指定は必要なのでエラーにならない。
    dt["country"] = "England"
    assert dt.columns == None
    assert dt.column_count == 0 # 未設定なので追加されない
