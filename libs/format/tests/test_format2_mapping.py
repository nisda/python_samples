...
from format_ex.format2 import data_mapping
...
def test_default():
    ...
    ret = data_mapping(template=template, data=INPUT_DATA)
    assert ret == expect
...
def test_assign_dtype_str():
    ...
    ret = data_mapping(template=template, data=INPUT_DATA, assign_dtype='str')
    assert ret == expect
    assert type(ret) == type(expect)
...
def test_error_handling(template, errors, expected):
    ret = data_mapping(template=template, data=INPUT_DATA, errors=errors)
    assert ret == expected
    assert type(ret) == type(expected)
...
@pytest.mark.parametrize(
    ...
    [
        # オリジナルタイプ
        pytest.param({"id": "{id}", "unknown": "{ID}"}, KeyError, "\"'ID'. occurred at path='.'<dict>\""),
        pytest.param({"id": "{id}", "unknown": "{type.999}"}, IndexError, "list index out of range. occurred at path='.type'<list>"),
        ...
        # フォーマット不備（str）
        pytest.param({"id": "{id}", "unknown": "{type{aaa}}"}, ValueError, "unexpected '{' in field name. occurred at expression='{type{aaa}}'"),
...
def test_error_handling(template, e_type, e_msg):
    with pytest.raises(Exception) as e:
        ret = data_mapping(template=template, data=INPUT_DATA, errors='raise')
